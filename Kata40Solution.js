...
class ConsumableUsers {
  constructor() {
    ...
    this.empty = false /// added this 
  }
  get nextUser() {
    if (this.users.length > 0) {
      return `user: ${this.users.shift()}`;
    }
    this.empty = true /// added this
    return void 0;
  }
  get anyLeft() {
    ///return this.users.length > 0; remove this
    return this.empty; /// added this 
  }
}
...
describe('Iterator usages', () => {
  ...
  beforeEach(function(){
    ...
    usersIterable = {};
    usersIterable[Symbol.iterator] = iteratorFunction; /// added this line
  });
  ...
  describe('fill the iterable with content using `ConsumableUsers`', function() {
    describe('using the iterator', function() {
      ...
      it('should return `done:true`, which means there are no more items', function() {
        iterator.next();
        iterator.next(); /// filled in the blank with .next()
    ...
    describe('using built-in constructs', function() {
      it('use `Array.from()` to convert an iterable to an array', function() {
        const users = Array.from(usersIterable); /// put usersIterable into the Array.from() method
        assert.deepEqual(users, ['user: Alice', 'user: Bob']);
      });
      it('use for-of to loop over an iterable', function() {
        const users = [];
        for (let user of usersIterable) users.push(user); /// changed in to of
        assert.deepEqual(users, ['user: Alice', 'user: Bob']);
      });
      it('use the spread-operator to convert/add iterable to an array', function() {
        const users = ['noname', ...usersIterable]; /// added the spread operator before usersIterable
        assert.deepEqual(users, ['noname', 'user: Alice', 'user: Bob']);
      });
      it('destructure an iterable like an array', function() {
        const [firstUser, secondUser] = usersIterable; /// changed the const from an object to an array
