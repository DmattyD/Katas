...
describe('`Array.prototype.findIndex` makes finding items in arrays easier', () => {
  it('takes a compare function, returns the index where it returned true', function() {
    const foundAt = [false, true].findIndex(item => item === true); /// added => item === true; this creates a function that returns an item = true
    assert.equal(foundAt, 1);
  });
  it('returns the first position it was found at', function() {
    const foundAt = [0, 1, 1, 1].findIndex(item => item === 1); /// asserted item === to 1
    assert.equal(foundAt, 1);
  });
  it('returns `-1` when nothing was found', function() {
    const foundAt = [1, 2, 3].findIndex(item => item > 4); /// changed item to something not in the array to return the -1
    assert.equal(foundAt, -1);
  });
  it('the findIndex callback gets the item, index and array as arguments', function() {
    const three = 3;
    const containsThree = arr => arr.indexOf(three) > -1;
    function theSecondThree(item, index, arr) { /// added item to the function () as a argument
  ...
  it('combined with destructuring complex compares become short', function() {
    ...
    const foundAt = [bob, alice].findIndex(({name:{length}}) => length > 3); /// changed name:length requirement to not 1
    assert.equal(foundAt, 1);
  });
});
