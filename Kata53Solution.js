...
describe('initialize a `Map`', function(){
  it('a `new Map()` is empty, has size=0', function() {
    const mapSize = new Map().size; /// set the Map().size to new Map().size to call a new map
    assert.equal(mapSize, 0);
  });
  it('init Map with `[[]]` has a size=1', function() {
    const mapSize = new Map([[]]).size; /// added a nested array into the new Map to create an array with 1 array inside
    assert.equal(mapSize, 1);
  });
  it('init a Map with `[[1]]` is the same as `map.set(1, void 0)`', function() {
    let map1 = new Map([[1]]); /// set map1 = new Map([[1]]);
  ...
  it('init Map with multiple key+value pairs', function() {
    const pair1 = [1, 'one'];
    const pair2 = [2, 'two'];
    const map = new Map([pair1, pair2]); /// called pair1 and pair2 in an array on new Map()
  ...
  it('keys are unique, the last one is used', function() {
    ...
    const pair3 = [1, 'eins'];
    const pair4 = [2, 'two'];
    const map = new Map([pair3, pair4]); /// called only pair3 and pair 4 in the map
  ...
  it('init Map from an Object, is a bit of work', function() {
    ...
    keys.forEach(key => map.set(key,obj[key])); /// set the map to the key and the obj in an array kally the keys
