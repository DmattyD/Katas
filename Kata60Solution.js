...
describe('`Reflect.getPrototypeOf` returns the prototype', function() {
  it('works like `Object.getPrototypeOf`', function() {
    ...
    const viaReflect = Reflect.getPrototypeOf({}); /// called an object in Reflect.getPrototypeOf();
    assert.strictEqual(viaObject, viaReflect);
  });
  it('throws a TypeError for a non-object', function() {
    let fn = () => { Reflect.getPrototypeOf() }; /// replaced the object with a string // numbers or boolean // removed the object
    assert.throws(fn, TypeError);
  });
  it('a `new Set()` has a prototype', function() {
    const aSet = new Set(); /// set aSet = new Set();
  ...
  it('works also for an old-style "class"', function() {
    function Klass() {}
    const proto = Reflect.getPrototypeOf( new Klass ()); /// called a new Klass() in reflect.getPrototype
    assert.equal(proto, Klass.prototype);
  });
  it('an array has a prototype too', function() {
    let arr = [];
    const expectedProto = Array.prototype; /// added .prototype after Array
